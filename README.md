...
* 本地环境：
  ...
  5. **练习rustlings**。使用VSCode等编辑器，进入clone下来的目录`2025s-rustling-xxxx`文件夹，执行`rustlings watch`依次查看完成情况，并依次完成对应的练习。 执行`rustlings run 练习名称`去运行对应练习，也可以使用`rustlings hint 练习名称`查看题解，也可以手动进入`exercises`目录检查所有练习。
